...
/* ----------------------------------------------------------------------
   ...
   part of the atom.in file where it reads the params of the atoms
------------------------------------------------------------------------- */
...
//  if (d_counter <= 0)
//	  error->one(FLERR, "Invalid division count in Division Counter section of atom data file");
...
bigint AtomVecBio::memory_usage()
{
  ...
  //dinika's edit
  //if (atom->memcheck("count")) bytes += memory->usage(d_counter,nmax);
