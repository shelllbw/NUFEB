...
void FixCohede::init_list(int id, NeighList *ptr)
{
      list = ptr;
}
...
void FixCohede::post_force(int vflag)
{
...
//		  else if (strcmp(bio->tname[ti],"diff") == 0 &&  strcmp(bio->tname[tj],"diff")== 0) ah = a6;
  ...
  if (rsq < (radsum + smax)*(radsum + smax)){
	    ...
	    del = r - radsum;
	    if (del > smin)
	      ccel = - ah*64*pow(radi,3)*pow(radj,3)*(del+radsum)/6.0/(del*del+2*radi*del+2*radj*del)/(del*del+2*radi*del+2*radj*del)/(del*del+2*radi*del+2*radj*del+4*radi*radj)/(del*del+2*radi*del+2*radj*del+4*radi*radj);
	    else if (del >= 0 && del <= smin)
	     // ccel = 0;
	      ccel = - ah*64*pow(radi,3)*pow(radj,3)*(smin+radsum)/6.0/(smin*smin+2*radi*smin+2*radj*smin)/(smin*smin+2*radi*smin+2*radj*smin)/(smin*smin+2*radi*smin+2*radj*smin+4*radi*radj)/(smin*smin+2*radi*smin+2*radj*smin+4*radi*radj);
	    else 
	      ccel = 0; 
	      rinv = 1/r;
...
}else error->all(FLERR,"invalid option for cohesivede force model");
}


/* ---------------------------------------------------------------------- */

void FixCohede::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixCohede::min_post_force(int vflag)
{
  post_force(vflag);
}

void FixCohede::extract_cohe(int *p_opt, double *p_ah, double *p_lam, 
...
int FixCohede::count_pairs(int flag)
{ 
...
void FixCohede::reallocate(int n)
{  
 ...
 // grow vector or array and indices array
  ...
  memory->create(array_local,nmax,nvalues,"Cohede:array");
