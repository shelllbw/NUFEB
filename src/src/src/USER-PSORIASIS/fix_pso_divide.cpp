...
FixPDivide::FixPDivide(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg) {
...
void FixPDivide::post_integrate() {
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    if (atom->mask[i] & groupbit) {
      ...
      if (type_id == 1 && canDivide == true) {
        ...
        double maxDivisionTime = 4;
        if (type_id == 2 && canDivide == true){
      	  //within its maximum proliferative life
      	  if (divisionTime < maxDivisionTime && canDivide == true){
        ...
        //differentiated cells - they don't divide
        //migrates to the top and then a percentage dies off
        if (type_id == 3){
      	  canDivide = false;
        }
...
int FixPDivide::modify_param(int narg, char **arg) {
